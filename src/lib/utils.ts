...
export const percentageChange = (currentPeriod:number, previousPeriod:number) => {
  if(previousPeriod === 0){
...
export const formatDate = (period?:period) => {
  if(!period?.from){
    ...
    return `${format(defaultFrom, "dd MMM")} - ${format(defaultTo, "dd MMM yyyy")}`
  }
  if(period.to){
    return `${format(period.from, "dd MMM")} - ${format(period.to, "dd MMM yyyy")}`
  }
...
export const getAllDays = 
  (
    ...
    endDay:Date 
      ...
      const incomeMap = new Map(activeDaysIncome.map(d => [d.date.toISOString().split("T")[0], d._sum.amount]));
      const expensesMap = new Map(activeDaysExpenses.map(d => [d.date.toISOString().split("T")[0], d._sum.amount]));
